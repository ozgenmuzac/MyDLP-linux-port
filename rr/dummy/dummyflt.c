...
#include "/home/ozgen/Desktop/virtual/redirfs-0.10/redirfs.h"
#include <net/sock.h>
...
enum redirfs_rv dummyflt_aio_write(redirfs_context context, struct redirfs_args *args)
{
	printk(KERN_INFO "DUMMYFLT_AIO_WRITE girdi!\n");
	return REDIRFS_CONTINUE;
}
...
enum redirfs_rv dummyflt_write(redirfs_context context, struct redirfs_args *args)
{
	...
	int chunk;
	char *written_data;
	const char __user *temp_buf;
	chunk = 512;
	
	msgSize = strlen(args->args.f_write.buf);
	temp_buf = args->args.f_write.buf;
	
	if(args->type.call == REDIRFS_PRECALL)
	{
		printk(KERN_INFO "message size dummy: %d", msgSize);
		printk(KERN_INFO "message size count: %zu", args->args.f_write.count);
		for(i = 0;;i += chunk)
		{
			if(msgSize - i <= chunk)
			{
				printk(KERN_INFO "1024'ten kucuk");
				written_data = kmalloc(sizeof(char)*(msgSize-i), GFP_KERNEL);
				if(written_data == NULL)
				{
					printk(KERN_INFO "kmalloc is failed!\n");
					goto exit;
				}
				if(copy_from_user(written_data, args->args.f_write.buf, (msgSize-i)))
				{
					printk(KERN_INFO "copy from user(kucuk) is failed\n");
					goto exit;
				}
				...
				nlh=nlmsg_put(skb_out,0,0,NLMSG_DONE, (msgSize-i), 0);  
				NETLINK_CB(skb_out).dst_group = 0; /* not in mcast group */
				strncpy(nlmsg_data(nlh), written_data, (msgSize-i));

				res=nlmsg_unicast(nl_sk,skb_out,pid);

				if(res != 0)
					printk(KERN_INFO "Error while sending bak to user\n");*/
				printk(KERN_INFO "kucuk: %s", written_data);
				args->args.f_write.buf += msgSize-i;
				kfree(written_data);
				break;
			}
			else
			{
				printk(KERN_INFO "1024'ten buyuk");
				written_data = kmalloc(sizeof(char)*chunk, GFP_KERNEL);
				if(written_data == NULL)
				{
					printk(KERN_INFO "kmalloc is failed!\n");
					goto exit;
				}
				if(copy_from_user(written_data, args->args.f_write.buf, chunk))
				{
					printk(KERN_INFO "copy from user(buyuk) is failed\n");
					goto exit;
				}
				skb_out = nlmsg_new(msgSize,0);
				if(!skb_out)
				...
				nlh=nlmsg_put(skb_out,0,0,NLMSG_DONE,1000,0);  
				NETLINK_CB(skb_out).dst_group = 0; /* not in mcast group */
				strncpy(nlmsg_data(nlh), written_data, 1000);

				res=nlmsg_unicast(nl_sk,skb_out,pid);

				if(res != 0)
					printk(KERN_INFO "Error while sending bak to user\n");*/
				printk(KERN_INFO "buyuk: %s", written_data);
				args->args.f_write.buf += chunk;
				kfree(written_data);
			}
		}
	}
	...
	args->args.f_write.buf = temp_buf;
...
static struct redirfs_op_info dummyflt_op_info[] = {
	...
	{REDIRFS_REG_FOP_AIO_WRITE, dummyflt_aio_write, dummyflt_aio_write},
	{REDIRFS_FIFO_FOP_AIO_WRITE, dummyflt_aio_write, dummyflt_aio_write},
	{REDIRFS_BLK_FOP_AIO_WRITE, dummyflt_aio_write, dummyflt_aio_write},
	{REDIRFS_CHR_FOP_AIO_WRITE, dummyflt_aio_write, dummyflt_aio_write},
	{REDIRFS_LNK_FOP_AIO_WRITE, dummyflt_aio_write, dummyflt_aio_write},
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,27)
